...
go test ./...                    # 全量测试（CI 在 ubuntu/windows/macos 三平台跑此命令）
go vet ./...                     # 静态检查
...
无 Makefile；质量门禁 = gofmt + go vet + staticcheck（CI 的 lint job 已接入，staticcheck 版本钉在 2025.1.1）。
...
internal/tokenpool/  凭证库 accounts.json v2（扁平 token 表 + primary 布尔列；失效凭证自动删除）
internal/updater/    自更新交互流程（检查→下载→SHA256 校验→安装），CLI/TUI 共用
internal/ui/         共用终端输出：日志行（ui.Log，签名兼容 engine.LogFunc）与 y/n 确认对话
...
- 引擎通过 `engine.LogFunc`（签名同 `ui.Log`）输出日志，不直接 fmt。
...
- **凭证库**：`accounts.json`（v2）是无别名的扁平 token 表，`primary` 布尔列标记至多一个主账号（exam 用）。login 走 `Store.LoginPrimary`：token 已存在→移主账号标记；已有主账号→原地替换其 token（防会话轮换累积僵尸）；否则追加并标记主账号。**失效即删除**：exam 收到 `engine.ErrLoginExpired` 后由前端 `RemoveTokenPersistent` 删主账号凭证；collect 通过 `CollectPoolOptions.OnTokenInvalid` 回调删对应 token。改动凭证逻辑必须补 `internal/tokenpool/accounts_test.go` 用例（含删除后再新增的序列）。并发写为单文件无锁设计：collect 运行中 login 以后写者为准，极端情况丢一次新增，重新 login 即可恢复。旧版 `.token/.tokens` 不迁移——重新 login；旧 v1 `accounts.json`（带别名）读取时自动升级为 v2。
...
- **凭证失效检测**：`sklclient.IsAuthError`（401 或报错文案含 token/未登录/过期等特征）命中时**凭证即被删除**——collect worker 经 OnTokenInvalid 回调删 token 后退出，exam 返回 `ErrLoginExpired` 由前端删主账号凭证；绝不进入冷却循环。特征列表在 `authErrPatterns`，网站恢复后应拿真实报错核对一次。
- **自更新**：编排统一在 `internal/updater.Run`（检查→单次确认→下载→SHA256 校验→自替换），TUI 可用 `--no-update-check` 跳过启动检查；安装助手协议两入口统一为 `apply-update --source --target` 子命令；Windows 下有删文件重试循环；SHA256SUMS 缺失用哨兵错误放行（历史版本），不匹配坚决中止。改动前读 UPDATE.md。
- **store 单连接**：`SetMaxOpenConns(1)`（SQLite 写锁所需），勿调大。

## 代码约定

- 用户可见的输出、日志、帮助文本均为**中文**；日志用 `ui.Log(level, ...)`（INFO/WARN/ERROR/OK/ROUND，支持 NO_COLOR），进度日志走 stdout、错误（fatalf/fatalErr）走 stderr。
